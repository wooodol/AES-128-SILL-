...
output reg [7:0] round_out
    ...
    assign sendin1 = gen_complete ^ encrypt_out;
    ...
    /*
    assign send = gen_complete & valid_round;
    */
    ...
    assign data_in_sbox1 = key[103:96];
    assign data_in_sbox2 = key[71:64];
    assign data_in_sbox3 = key[39:32];
    assign data_in_sbox4 = key[7:0];
        ...
        if(!rst)begin
            ...
            if(gen | re_gen)begin
                case(cnt)
                    ...
                    4'b0001: cnt <= cnt + 1;
                    4'b0010: begin
                        {key[127:120],key[95:88],key[63:56],key[31:24]} <=
                        
                        {key[127:120] ^ rcon(round+1) ^ data_out_sbox2, key[95:88] ^ 8'b0 ^ data_out_sbox3,
                         key[63:56] ^ 8'b0 ^ data_out_sbox4, key[31:24] ^ 8'b0 ^ data_out_sbox1};    
                         cnt <= cnt + 1;
                    end
                    4'b0011: begin
                        {key[119:112],key[87:80],key[55:48],key[23:16]} <= 
                        
                        {key[119:112] ^ key[127:120], key[87:80] ^ key[95:88],
                         key[55:48] ^ key[63:56], key[31:24] ^ key[23:16]};
                         cnt <= cnt + 1;
                    end
                    4'b0100:begin
                        {key[111:104],key[79:72],key[47:40],key[15:8]} <=
                        
                        {key[111:104] ^ key[119:112], key[79:72] ^ key[87:80],
                         key[47:40] ^ key[55:48], key[15:8] ^ key[23:16]};
                         cnt <= cnt + 1;
                    end
                    4'b0101:begin
                        {key[103:96],key[71:64],key[39:32],key[7:0]} <=
                        
                        {key[103:96] ^ key[111:104], key[71:64] ^ key[79:72],
                         key[39:32] ^ key[47:40], key[7:0] ^ key[15:8]};
                         cnt <= 4'b0;
                         gen_complete <= 1'b1;
                         gen <= 1'b0;
                         re_gen <= 1'b0;
